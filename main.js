...
ctrl.onchange = (/** @type {OrbitState} */ state) =>
  viewState.saveCamera(state);
ctrl.oninput = (/** @type {OrbitState} */ state) => updateFromCtrl(state);

gizmo.onRotationRequested = (/** @type {string} */ cam) =>
  ctrl.animateToCommonCamera(cam);
...
async function initFs() {
  ...
  try {
    sw = await registerServiceWorker(
      `bundle.fs-serviceworker.js?prefix=${scope}swfs/`,
      getFileWrapper,
      {
        scope,
        prefix: scope + "swfs/",
...
const workerApi = /** @type {JscadWorker} */ (
  messageProxy(worker, handlers, { onJobCount: trackJobs })
);

const progress = /** @type {HTMLProgressElement} */ (
  byId("progress").querySelector("progress")
);
...
const jscadScript = async ({
  ...
  try {
    ...
    let tmp = genParams({
      target: byId("paramsDiv"),
      params: result.def || [],
      callback: paramChangeCallback,
      pauseAnim: pauseAnimCallback,
      startAnim: startAnimCallback,
    });
